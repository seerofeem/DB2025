...
SELECT e.department,
       ...
       (SELECT e2.first_name || ' ' || e2.last_name
        ...
        ORDER BY e2.salary DESC LIMIT 1) AS highest_paid_employee,
       GREATEST(MAX(e.salary), MIN(e.salary)) AS greatest_salary,
       LEAST(MAX(e.salary), MIN(e.salary)) AS least_salary
FROM employees e
